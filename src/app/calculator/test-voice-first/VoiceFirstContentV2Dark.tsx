              ...
              <CardContent className="pt-6 sm:pt-8 lg:pt-10">
                ...
                {isConnected && (
                  <div className="space-y-6">
                    {/* Friendly Progress */}
                    {progress && !isComplete && (
                      <div className="bg-gradient-to-r from-slate-700 to-slate-600 rounded-2xl p-5 space-y-3">
                        <div className="flex items-center justify-between">
                          <span className="text-sm font-bold text-gray-200">
                            Step {progress.current} of {progress.total}
                          </span>
                          <Badge className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white border-0 rounded-full px-3">
                            {Math.round((progress.current / progress.total) * 100)}%
                          </Badge>
                        </div>
                        <Progress
                          value={(progress.current / progress.total) * 100}
                          className="h-4 bg-slate-900/50 rounded-full"
                        />
                        <p className="text-xs text-gray-400 mt-2">We're making great progress!</p>
                      </div>
                    )}
                  ...
                  <div className="space-y-5">
                    {batchPrompts.map((batch, idx) => (
                      <div key={batch.batchId} className="relative pl-6">
                        <div className="absolute left-0 top-0 w-10 h-10 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-600 flex items-center justify-center text-white font-bold shadow-lg">
                          {batch.batchIndex + 1}
                        </div>
                        <div className="pl-6">
                          <h4 className="font-bold text-gray-200 mb-2">{batch.batchTitle}</h4>
                          <ul className="space-y-1.5">
                            {batch.questions.map(q => (
                              <li key={q.id} className="text-sm text-gray-400 flex items-start gap-2">
